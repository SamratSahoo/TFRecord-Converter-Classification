#! /usr/env/bin python3
# In coutesy of https://gist.github.com/damienpontifex/1a3a7f1ff7a572b6b5e69916c0eaf932
import argparse
import os
import sys
import glob
...
def convert_to_tfrecord(dataset_name, data_directory, class_map, segments=1, directories_as_labels=True,
    ...
    classes = (os.path.basename(os.path.dirname(name)) for name in filenames) if directories_as_labels else [
                                                                                                                None] * len(
        filenames)
    ...
    print(os.path.join(data_directory, files), segments, samples_per_segment)
